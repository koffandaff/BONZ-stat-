...
const fieldSx = {
  mb: 3,
  "& .MuiInput-root": { color: "#E8E2D9", fontFamily: "var(--font-space-mono)", fontSize: "16px" },
  "& .MuiInputLabel-root": { color: "#444", fontFamily: "var(--font-barlow)", letterSpacing: "0.25em", textTransform: "uppercase", fontSize: "13px" },
  ...
  return (
    <section id="contact" className="rg" style={{
      ...
      <div className="rp" style={{ position: "relative", zIndex: 1, padding: "80px 60px" }}>
        <div style={{ fontFamily: "var(--font-barlow)", fontSize: "12px", color: "#8B0000", letterSpacing: "0.5em", textTransform: "uppercase", marginBottom: "24px" }}>
          GET IN TOUCH
        </div>
        ...
        <p style={{ fontFamily: "var(--font-space-mono)", fontSize: "14px", color: "#444", lineHeight: 2.1, maxWidth: "300px", marginBottom: "36px" }}>
          ...
          {[
            ...
            <motion.div key={lnk.label} initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
              <a href={lnk.href} target="_blank" rel="noopener noreferrer" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", border: "1px solid #1a1a1a", textDecoration: "none", color: "#E8E2D9", transition: "border-color 0.2s, color 0.2s" }}
                ...
                <span style={{ fontFamily: "var(--font-barlow)", fontSize: "13px", textTransform: "uppercase", letterSpacing: "0.1em" }}>{lnk.label}</span>
                <span>→</span>
              </a>
            </motion.div>
          ))}
        ...
        <AnimatePresence mode="wait">
          {!sent ? (
            <motion.div key="form" exit={{ opacity: 0, scale: 0.96 }}>
              ...
              <Magnet padding={16} disabled={false} magnetStrength={2}>
                <button onClick={() => setSent(true)} style={{
                  fontFamily: "var(--font-barlow)", fontSize: "13px", textTransform: "uppercase", letterSpacing: "0.4em",
            ...
            <motion.div key="success" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <div style={{ fontFamily: "var(--font-space-mono)", fontSize: "14px", color: "#444", marginTop: "8px" }}>We&apos;ll be in touch.</div>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
